...
Input parameters : handle on bitstream iso11172 (DOS -> pointeur sur FILE on bitstream
									global handle video buffer    ( pointeur on u8)
...
// open a file
GLOBAL_LOCK(lptr_Iso11172, ph_Iso11172,"..\\hedit\\b.mpg","Iso11172_stream: lptr_Iso11172=NULL","hedit",ret,"rb");
...
//	BitPos ++;
//}
NB_TO_STRING(StreamPtr,BitPos,string,length,&i);
/*				fseek(StreamPtr,BitPos/8,SEEK_SET);
				i=0;
				while(i<length)
				{
				 fread(&sremi,1,1,(FILE *)StreamPtr);
				 do
				 {
					itoa((((unsigned char)sremi)>>(7-(BitPos+i)%8))&0x01,&string[i],10);
					i++;
				 } while((BitPos%8+i)%8!=0);
				}
				BitPos+=length;
...
if (!ret) /* mux_rate */
{
	*pBitPos+=22; /* bypass mux_rate (22 bytes) */
	if (!Nextbits((LPBYTE) lpStream,MARKER_BIT,*pBitPos))
	{
		ret = 2;
...
} /* end SYSTEM_HEADER_START_CODE */

while(!ret && Nextbits((LPBYTE) lpStream,PACKET_START_CODE_PREFIX,*pBitPos) &&\
		 !Nextbits((LPBYTE) lpStream,PACK_START_CODE,*pBitPos))
...
*pBitPos+=16; /* bypass header_length 16 bits */
...
/* store streamid */
/*GET_NB_FROM_STREAM( (BYTE *)&stream_id,
										(DWORD *)pBitPos,
										(BYTE)i);*/

READ_NB_FROM_STREAM(lpStream,pBitPos,8, &stream_id);
//for(i=0;i<8;i++)
//{
...
/* store packet length */
READ_NB_FROM_STREAM(lpStream,pBitPos,16, &packet_length);
//for(i=0;i<16;i++)
//{
...
if (strcmp(PRIVATE_STREAM_2,s))
{
	...
	if (strcmp(PRIVATE_STREAM_2,s))
	{
		/* bit packet restant : packetlength -(BitPos-BitPosInit) to be jumped */
//		*pBitPos=packet_length*8+BitPosInit;
		while(!Nextbits(lpStream,PACKET_START_CODE_PREFIX,*pBitPos))
			*pBitPos+=8;

 ...
 * ---> Add a constructor that can read in a graph from a file. The file format will be an initial integer that is the node size of the graph and the further values will be integer triples: (i, j, cost). See Sample Test Data here.
...
void PriorityQueue::print(){
	for (vector<QueueElement>::iterator p = queue.begin(); p != queue.end(); ++p){
		cout << p.node.v << "; ";
	}
	cout << endln;
}
...
bool copy_file_2_graph(int& nbNodes, Graph& g)
{
	string filename = "20131111_Courseracplusplus4c_homeworks_Homework3_SampleTestData_mst_data.txt";
	Vertice v1, v2;
	Cost c;
	...
	// Then read each edge and fill the graph accordingly.
	while(!ifp.eof())
	{
		...
		// now fill graph accordingly g
		// g.G[v1.v][v2.v].c = g.G[v2.v][v1.v].c ;
		g.add(v1, v2, c);
...
//		cout << shortestPathAverage(s2) << endl;
		...
		cout << "WEEK3" << endl << "======" << endl << endl ;
		int nbNodes(0);
		Graph g3 = Graph();
		
//		g3.print();
		copy_file_2_graph(nbNodes, g3);
		g3.print(nbNodes);

		/* WEEK3 :
		 * * Implement either the Prim Minimum Spanning Tree Algorithm.
		*/		
		PriorityQueue pq=PriorityQueue(); // ClosedSet
		PriorityQueue oq=PriorityQueue(); // OpenSet
		
		// Here should build OpenQueue with all nodes.
		// void neighbors(Vertice x, vector<Vertice>& nodes); // neighbors (G, x): lists all nodes y such that there is an edge from x to y.
		// scroll all nodes of the graph g3 :
		for (int i=0; i<nbNodes;i++)
		{
			vector<Vertice> neigh(0);
			g3.neighbors(Vertice(i), neigh);
			if(!neigh.empty())
			{
					// add vertice to open list !
					// PriorityQueue::insert(QueueElement q)
					oq.insert(QueueElement(Vertice(i),Cost(0)));
			}
		}
		
		
		//		
		/* Implementation Prim algo first */
		
		/* dist : array of distances from the source to each vertex */
		/*  Initialization: set every distance to INFINITY until we
			discover a way to link a vertex to the spanning tree */
		vector<Cost> dist(nbNodes, INFINITY);
		
		/* edges: array indicating, for a given vertex, which vertex
		in the tree it is closest to */
		vector<Vertice> edges(nbNodes, NULL); 
		...
		while(!isfinished)
		{
			/* pick the vertex, v, in U with the shortest edge to the group of vertices
				in the spanning tree add v to MST */
			Vertice v0 = MST.back();
			// ie get U (list or heap unfinished vertices)
			//void neighbors(Vertice x, vector<Vertice>& nodes); // neighbors (G, x): lists all nodes y such that there is an edge from x to y.
			g3.neighbors(v0, U);
			// search the shortest neighbor to initial ==> vertex v. Add v to MST and I guess its cost to dist[].
